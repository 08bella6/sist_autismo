...
La Inteligencia Artificial se define como el campo de la ciencia computacional dedicado al desarrollo de sistemas capaces de realizar tareas que, tradicionalmente, requerirían de la inteligencia humana, tales como el razonamiento, el aprendizaje y la percepción visual \cite{russell_norvig_ai_2020}. En el ámbito de la ingeniería, la IA ha evolucionado de sistemas basados en reglas lógicas hacia modelos capaces de extraer conocimiento directamente de los datos \cite{mitchell_ml_1997}. Esta evolución ha permitido que la disciplina pase de ser un conjunto de heurísticas rígidas a convertirse en un marco robusto de aprendizaje estadístico y computacional.

Actualmente, la IA se divide en diversas áreas de especialización, siendo el Aprendizaje Automático (Machine Learning) y el Aprendizaje Profundo (Deep Learning) las ramas de mayor impacto en la ingeniería moderna. Estos sistemas no solo procesan información, sino que son capaces de generalizar soluciones a partir de patrones complejos identificados en conjuntos de datos masivos \cite{goodfellow_dl_2016}. En el área de la salud, este enfoque ha permitido el descubrimiento de biomarcadores y patrones conductuales que resultan imperceptibles para los métodos de observación tradicionales, validando el uso de la IA como una herramienta de apoyo crítico en el diagnóstico clínico \cite{sherkatghanad_autism_2020}.
\subsubsection{Aprendizaje automático}

El Aprendizaje Automático (Machine Learning) se define como una subdisciplina fundamental de la Inteligencia Artificial cuyo objetivo es desarrollar algoritmos y modelos estadísticos que otorgan a los sistemas computacionales la capacidad de aprender a partir de datos empíricos, identificando patrones complejos y tomando decisiones con una intervención humana mínima \cite{bishop_prml_2006}. A diferencia de la programación de software tradicional, donde un ingeniero codifica reglas estáticas y condicionales de forma explícita, los modelos de aprendizaje automático infieren sus propias reglas matemáticas basándose en la exposición continua a ejemplos históricos.

En el ámbito de la ingeniería aplicada a la salud y el diagnóstico automatizado, el paradigma más utilizado es el \textbf{aprendizaje supervisado}. En este enfoque, el algoritmo se entrena utilizando un conjunto de datos rigurosamente etiquetado por especialistas clínicos; por ejemplo, secuencias de video clasificadas según la presencia o ausencia de marcadores motores atípicos. A través de un proceso iterativo de optimización, el sistema ajusta sus parámetros internos para generalizar el conocimiento adquirido, logrando predecir con alta precisión el diagnóstico de nuevos pacientes no vistos anteriormente. Esta capacidad de generalización convierte al Machine Learning en un pilar esencial para agilizar el tamizaje médico y reducir la carga sobre los sistemas de salud pública \cite{aws_ml_2023}.


\subsubsection{Visión artificial}

La Visión Artificial (Computer Vision) se define como el campo interdisciplinario de la Inteligencia Artificial que busca otorgar a los sistemas computacionales la capacidad de adquirir, procesar, analizar y comprender el contenido visual del mundo real a partir de imágenes digitales y secuencias de video.

A nivel computacional, una computadora no ve una imagen como nosotros; ve una matriz numérica (una cuadrícula de valores donde cada número representa la intensidad de un píxel). El objetivo de la Visión Artificial es transformar esas matrices de números en decisiones o predicciones semánticas \cite{szeliski_cv_2022}.

Para entender su evolución y su estado actual, podemos dividir la Visión Artificial en dos grandes enfoques que detallan su flujo de trabajo:


\begin{itemize}
    \item \textbf{Procesamiento:} Mejorar la calidad de la imagen aplicando filtros matemáticos (por ejemplo, suavizado gaussiano) para eliminar el ruido o mejorar el contraste \cite{forsyth_ponce_cv_2011}.
    \item \textbf{Extracción de Características espaciales:} Los ingenieros debían diseñar algoritmos matemáticos específicos (como SIFT, HOG o detección de bordes de Canny) para buscar geometrías particulares, líneas, esquinas o texturas en la imagen \cite{szeliski_cv_2022}, \cite{forsyth_ponce_cv_2011}.
...
El análisis de video (Video Understanding o Action Recognition) requiere procesar la dimensión temporal. Aquí es donde la extracción espacial de la CNN se acopla con modelos de memoria (como las redes LSTM) para evaluar cómo la postura detectada en el primer fotograma evoluciona dinámicamente hacia el segundo, el tercero y así sucesivamente, permitiendo que la máquina reconozca el concepto de "movimiento repetitivo" a lo largo del tiempo \cite{simonyan_zisserman_twostream_2014}.
\subsubsection{Arquitectura Híbrida CNN-LSTM para Reconocimiento de Patrones}

El análisis de video para la detección de comportamientos asociados al TEA implica manejar información espacial (posturas y expresiones en cada fotograma) y temporal (cómo esos movimientos se repiten o evolucionan), lo que representa un desafío de alta dimensionalidad. Para abordarlo, se propone una arquitectura híbrida basada en CNN y LSTM \cite{orozco_cnnlstm_2021}.
...
\end{itemize}
En esta fase, el modelo generará un vector de características unidimensionales por cada instante de tiempo, eliminando información irrelevante del entorno \cite{pajares_dl_2018}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{web_autismo.png}
    \caption{Diseño de la interfaz}
    \label{fig:web_tea}
\end{figure}
...
Este enfoque desacoplado permitirá que el modelo de inteligencia artificial evolucione independientemente del sistema web, facilitando futuras mejoras en términos de precisión, escalabilidad y mantenimiento del sistema \cite{bass_architecture_2012}.
...
Asimismo, se establecerán políticas de protección de datos que garanticen la confidencialidad de la información, incluyendo el almacenamiento seguro de los videos y resultados generados. Este enfoque será coherente con principios internacionales de protección de datos y ética en sistemas de salud digital \cite{who_ai_health_2021}.
...
Además, el uso de técnicas de visualización interactiva contribuirá a reducir la ambigüedad en la interpretación de los resultados, transformando datos complejos en información clara y accesible \cite{ware_infovis_2013}.
...
Los resultados del estudio muestran que este tipo de enfoque permite detectar diferencias significativas entre niños con y sin TEA, ya que los patrones de interacción con los bloques evidencian comportamientos repetitivos, rigidez en la ejecución o dificultades en la planificación. Además, se destaca que la combinación de modelos capaces de capturar características espaciales y temporales mejora la precisión del sistema en la identificación de estos patrones conductuales \cite{vaswani_attention_2024}.
